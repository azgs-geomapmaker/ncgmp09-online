...
class GdbValidator():
    acceptedLayers = [ cls._meta.object_name for cls in get_models() if cls._meta.app_label == "ncgmp" and cls._meta.object_name != "GeoMap" ]
    ...
    def __init__(self, fgdb):
        ...
        self.tablesAreValid = self.validateTables()
        self.fieldsAreValid = self.validateFields()
        self.uniqueKeysAreValid = self.validateUniqueKeys()
        self.foreignKeysAreValid = self.validateForeignKeys()
        
        if self.tablesAreValid and self.fieldsAreValid and self.uniqueKeysAreValid and self.foreignKeysAreValid:
            self.fgdbIsValid = True
        else:
            self.fgdbIsValid = False
    ...
    def validateFields(self):
        output = True
        for layer in self.fgdb:
            requiredFields = set(self.requiredFields[layer.name])
            requiredFieldsPresent = requiredFields.intersection(layer.fields)
            if requiredFieldsPresent != requiredFields:
                missingFields = list(requiredFields.difference(requiredFieldsPresent))
                self.addMissingFields(layer.name, missingFields)
                output = False            
        return output                            
    ...
    def validateUniqueKeys(self):
        ...
        for layerName in self.acceptedLayers:
            ...
            uniqueFields = [ fld for fld in clsFields if fld.name != "id" and fld.unique ]
            for field in uniqueFields:
                gdalLayer = getLayerByName(layerName, self.fgdb)
                values = [ feature[field] for feature in gdalLayer ]
                countedValues = Counter(values)
                repeats = [ i for i in countedValues if countedValues[i] > 1 ]
                if len(repeats) > 0:
                    self.addRepeatedUniqueValues(layerName, field, repeats)
                    output = False
        return output
    ...
    def validateForeignKeys(self):
        ...
        for layerName in self.acceptedLayers:
            cls = get_model("ncgmp", layerName)
            clsFields = cls._meta.fields
            fkFields = [ field for field in clsFields if isinstance(field, ForeignKey) ]
    ...
    class Logger():
        missingTables = [] 
        missingFields = {}
        repeatedUniqueValues = {}
        missingForeignKeys = {}
        
        def consoleMessage(self):
            tableMessage = "\n".join(self.missingTables)
            fieldMessage = "\n".join([tableName + ": " + ", ".join(self.logs.missingFields[tableName]) for tableName in self.logs.missingFields.keys()])
